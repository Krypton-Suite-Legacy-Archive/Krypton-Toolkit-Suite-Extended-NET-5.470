...
{
    /// <summary>
	/// A label that allows use of HTML to format the text. Adapted from (https://github.com/Nexus-Mods/Nexus-Mod-Manager/blob/master/UI/Controls/HtmlLabel.cs)
	/// </summary>
    [ToolboxBitmap(typeof(KryptonLabel))]
    [Description("A label that allows use of HTML to format the text. Adapted from (https://github.com/Nexus-Mods/Nexus-Mod-Manager/blob/master/UI/Controls/HtmlLabel.cs)")]
    public class HTMLLabel : WebBrowser
    {
        ...
        protected override void OnNavigating(WebBrowserNavigatingEventArgs e)
        {
            if (!"blank".Equals(e.Url.LocalPath, StringComparison.OrdinalIgnoreCase))
            {
                ...
                if ("about".Equals(newURL.Scheme, StringComparison.OrdinalIgnoreCase))
                {
                    newURL = new Uri(Uri.UriSchemeHttp + Uri.SchemeDelimiter + newURL.LocalPath);
                }
