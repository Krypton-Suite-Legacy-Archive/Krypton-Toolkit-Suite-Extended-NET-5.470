...
namespace ExtendedColourControls.Controls
{
    ...
    public class KryptonColourGrid : Control, IColourEditor
    {
        #region Constants
        ...
        /// <summary>
        /// Returns the <see cref="Rectangle"/> describing the bounds of a single colour cell
        /// </summary>
        /// <exception cref="ArgumentOutOfRangeException">Thrown when one or more arguments are outside the
        /// required range.</exception>
        /// <param name="index">Zero-based index of the colour cell to return.</param>
        /// <returns>
        /// The cell bounds.
        /// </returns>
            ...
            if (this.AllowPainting)
            {
                ...
                if (this.CustomColours.Count != 0 && this.ShowCustomColours)
                {
                    ...
                    for (int i = 0; i < this.CustomColours.Count; i++)
                    {
                        ...
                        if (_colourRegions.TryGetValue(colourCount + i, out bounds) && e.ClipRectangle.IntersectsWith(bounds))
                        {
                            this.PaintCell(e, i, colourCount + i, this.CustomColours[i], bounds);
                        }
                    }
                }
        ...
        if (this.HotIndex != InvalidIndex)
        {
          ...
          {
            name = this.GetColour(this.HotIndex).Name;
          }
        }
        else
        {
